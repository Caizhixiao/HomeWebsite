...
$findUserID = $_POST["findUserID"];
if (filterString($userid) != $userid || filterString($token) != $token || filterString($accessToken) != $accessToken || filterString($findUserID) != $findUserID){
    echo '{"code":-403,"text":"含非法字符或字符段!"}';
    return ;
}

$findUserID = explode(",",$findUserID);
// 创建连接
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        while (in_array($row["uid"],$findUserID)){
            $findUserID[array_search($row["uid"],$findUserID)] = $row["username"];
        }
        if ($userid == $row["uid"]){
            $find = 1;
            $key = $row["dec_pwd"];
...
$conn->close();
if ($success == 1){
    $res = ["code" => 200 , "result" => $findUserID];
