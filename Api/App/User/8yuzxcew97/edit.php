...
$edinfo = json_decode($_POST["edit"],true);
$medid = $_POST["med_id"];
if (filterString($userid) != $userid || filterString($token) != $token || filterString($accessToken) != $accessToken || filterString($medid) != $medid){
...
$conn = null;
$dsn = 'mysql:host=localhost;dbname=home_caizhixiao'; // 根据自己的情况修改主机名、数据库名等信息
try {
    $pdo = new PDO($dsn, 'home_caizhixiao', 'zkfc8wdswf5Dn64x'); // 根据自己的情况修改用户名和密码
} catch (Exception $e) {
    die('{"code":-200,"text":"无法连接到数据库: ' . $e->getMessage() . '"}');
}
...
foreach ($edinfo as $key => $value) {
    $val = $value["new"];
    $sql .= "`{$key}` = :{$key},";
}
$sql = rtrim($sql, ',');
$sql .= " WHERE MedicineID = :id";
$statement = $pdo->prepare($sql);
foreach ($edinfo as $key => $value) {
    $val = $value["new"];
    $statement->bindValue(":{$key}", $val);
}
$statement->bindValue(":id", $medid);
if (!$statement->execute()) {
    die('{"code":-200,"text":"更新失败: ' . implode(', ', $statement->errorInfo()) . '"}' );
} else {
   echo '{"code":200,"text":"修改成功"}';
}
?>